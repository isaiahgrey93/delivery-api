...
const cost_per_pound = 0.35;
const cost_per_cubic_foot = 0.35;
...
module.exports = {
                ...
                ORM.thinky.r
                    ...
                    .and(
                        ORM.thinky.r
                            .row("requester_id")
                            .eq(user)
                            .or(ORM.thinky.r.row("driver_id").eq(user))
                    )
            )
            .then(drives => {
                return drives
                    .map(({ price, start_time, end_time }) => {
                        return {
                            price,
                            duration: new Date(end_time).getTime() -
    ...
    getEstimate: ({ items, distance }) => {
        ...
        estimate.items = items.map(
            ({ height, width, length, weight, quantity }) => {
    ...
    getGeoPoints: drive => {
        ...
        addresses = addresses.map(address =>
            clientGeocoder.geocode(address).asPromise()
        );
        ...
        return Promise.all(addresses)
            .then(points => {
                ...
                return clientDistanceMatrix
                    .distanceMatrix(data)
                    .asPromise()
                    .then(res => {
                        drive.route.distance = Number(
                            res.json.rows[0].elements[0].distance.text.split(
                                " "
                            )[0]
                        );
                        return drive;
    ...
    getTripDistance: ({ route, items }) => {
        ...
        return clientDistanceMatrix
            .distanceMatrix(drive)
            .asPromise()
            .then(res => {
                return {
                    items,
                    distance: Number(
                        res.json.rows[0].elements[0].distance.text.split(" ")[0]
                    )
                };
            });
    },
