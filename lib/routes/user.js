    ...
    validate: {
        payload: {
            ...
            firstname: Joi.string()
                .description("The firstname of the account user.")
                .example("John"),
            middle_initial: Joi.string()
                .description("The middle initial of the account user.")
                .example("A"),
            lastname: Joi.string()
                .description("The lastname of the account user.")
                .example("Doe"),
            nickname: Joi.string()
                .description("The nickanme of the account user.")
                .example("Jo"),
            profile_photo: Joi.any()
                .description("The unique id of the photo in S3")
                .example("[UUID]"),
            phone: Joi.string()
                .description("The phone number of the account")
                .example("555-555-5555"),
            dob: Joi.string()
                .description("The account user date of birth")
                .example("12-15-78"),
            rating: Joi.number()
                .description("The rating of a driver")
                .notes("Applies to drivers only"),
            drivers_license: Joi.object().keys({
                expiry_month: Joi.string()
                    .description("Drivers license expiration month.")
                    .example("6"),
                expiry_year: Joi.string()
                    .description("Drivers license expiration year.")
                    .example("19"),
                number: Joi.string()
                    .description("Drivers license number.")
                    .example("8LK7 4K6"),
                photo: Joi.any()
                    .description("Drivers license photo.")
                    .example("[UUID]"),
                state: Joi.string()
                    .description("Drivers license issue state.")
                    .example("CA")
            }),
            address: {
                street: Joi.string()
                    .description("Street address")
                    .example("412 Washington"),
                city: Joi.string()
            ...
            geo: Joi.object().description("Last logged location of user")
        }
    },
    pre: UserPrehandlers,
...
module.exports.login = {
    ...
    validate: {
        query: {
            populate: Joi.string()
                .description("User relations to populate")
                .example("?populate=vehicle")
        },
        payload: {
            email: Joi.string()
                .email()
...
module.exports.update = {
    ...
    validate: {
        ...
        params: {
            user_id: Joi.string()
                .required()
                .description("Unqiue id of the user to update.")
                .example("[UUID]")
        },
        query: {
            populate: Joi.string()
...
module.exports.remove = {
    ...
    validate: {
        params: {
            user_id: Joi.string()
                .required()
                .description("Unqiue id of the user to remove.")
                .example("[UUID]")
        }
    },
    tags: ["api"],
...
module.exports.password_reset_request = {
    ...
    validate: {
        params: {
            email: Joi.string()
                .required()
                .description("Email address of the requested password reset.")
        }
    },
    tags: ["api"],
    handler: function(request, reply) {
            ...
            .then(user => {
                if (!user)
                    throw Boom.badRequest(
                        `Email <${email}> is not registered to an account.`
                    );
                console.log(user);
...
module.exports.reset_password = {
    ...
    validate: {
        payload: {
            password: Joi.string()
                .min(6)
                .required()
                .description("New password to be set in reset password.")
        },
        params: {
            email: Joi.string()
                .required()
    ...
    handler: function(request, reply) {
        ...
        console.log("resetting password", email, new_password);
        this.core.user
            .findByEmail(email)
            .then(user => {
...
module.exports.getById = {
    validate: {
        ...
        query: {
            populate: Joi.string().description(
                'Option to populate user relationships. Options: ["vehicle", "categories"]'
            )
        }
    },
    tags: ["api"],
...
module.exports.filterBy = {
    validate: {
        query: {
            populate: Joi.string()
                .description(
                    'Option to populate category relationships. Options: ["driver", "requester", "support_calls"]'
                )
                .example("?populate=driver,requester"),
            geometry: Joi.array()
                .description(
                    "Longitude & latitude of the center the radius to search from"
                )
                .example([-116.55712299999999, 48.2833078]),
            distance: Joi.number()
                .description(
                    "The distance to search from the geo point passed in."
                )
                .positive()
        }
    },
