...
module.exports.update = {
    ...
    handler: function(request, reply) {
        ...
        .then((res) => this.core.model('User').findById(res.id, {
            populate: this.utils.model.populate(relations),
            without: {
                password: true
            }
...
module.exports.reset_password = {
    ...
    validate: {
        payload: {
            password: Joi.string().min(6).required().description('New password to be set in reset password.')
        },
        params: {
            email: Joi.string().required().description('Email address of the requested password reset.'),
        }
...
module.exports.filterBy = {
    validate: {
        query: {
            populate: Joi.string().description('Option to populate category relationships. Options: ["driver", "requester", "support_calls"]').example('?populate=driver,requester'),
            geometry: Joi.array().description('Longitude & latitude of the center the radius to search from').example([-116.55712299999999, 48.2833078]),
            distance: Joi.number().description('The distance to search from the geo point passed in.').positive()
        },
    },
    tags: ['api'],
    handler: function(request, reply) {
