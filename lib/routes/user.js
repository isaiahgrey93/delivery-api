  ...
  handler: function(request, reply) {
    ...
    this.core.model('User').create(user)
      .then((user) => {
        ...
        reply(account)
          .header('Authorization', this.utils.user.grantJSONWebToken(account))
      })
      .catch((err) => reply(err));
  }
};
    ...
    this.core.user.findByEmail(credentials.email)
      ...
      .then((res) => {
        if (res.isBoom) return reply(res);
          ...
          reply(res)
            .header('Authorization', this.utils.user.grantJSONWebToken(res));
        }
      })
      .catch((err) => reply(err));
  }
...
module.exports.password_reset_request = {
  ...
  handler: function (request, reply) {
    let user = {
      ...
      password_reset: this.utils.user.generatePasswordResetToken()
    }
...
module.exports.reset_password = {
  ...
  validate: {
    payload: {
      new: Joi.string().min(6).required().description('User\'s new password.')
    },
    params: {
      user_id: Joi.string().required().description('User\'s unique id.'),
      token_id: Joi.string().required().description('Unique token id generated from requested passsword reset.')
    }
  },
  tags: ['api'],
  handler: function (request, reply) {
    ...
    this.core.user.findByResetToken(token)
      .then((user) => {
        if (!user) throw Boom.unauthorized(`Token <${token}> is invalid.`);
