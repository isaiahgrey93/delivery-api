...
module.exports.accept = {
    ...
    handler: function (request, reply) {
        ...
        this.core.model('Drive').findById(id)
        .then((drive) => {
            if (drive.driver_id !== 'N/A') throw Boom.badRequest('Drive is no longer available.')

            try {
                this.core.support.sms.send(drive.customer.phone, "Your delivery has been accepted by a JOEY driver. The driver will be calling you soon to coordinate your delivery.")
            } catch (e) {
                console.log(e)
            }

            return this.core.model('Drive').update({ id, driver_id, status: 'accepted' })
        })
        .then((res) => {
...
module.exports.charge = {
    ...
    handler: function (request, reply) {
        ...
        .then((charge) => {
            ...
            if(!charge.paid) return Boom.badRequest(`<Unable to process payment for drive <${id}>.`);

            let drive = {
                id: id,
                status: 'available',
                payment: {
                    driver_payout: this.utils.stripe.getPayout('driver', charge.amount),
                    joey_payout: this.utils.stripe.getPayout('joey', charge.amount),
                    charge_id: charge.id,
                }
            }
...
module.exports.getStats = {
    ...
    validate: {
        params: {
            user_id: Joi.string().required().description('The unique id of the user to find drive stats for.').example('[UUID]')
        },
        query: {
            start: Joi.number().description('Date range start point, as a timestamp').example(1482286102098),
            end: Joi.number().description('Date range end point, as a timestamp').example(1482286102507),
        }
    },
    tags: ['api'],
    handler: function (request, reply) {
        ...
        if (start && end) {
            this.core.drive.getUserDriveStats(id, start, end)
                .then((drive) => reply(drive))
                .catch((err) => reply(err));
        } else {
            ...
            this.core.drive.getUserDriveStats(id, ts() - day, ts() + day)
                .then((today) => {
                    stats.today = today;
                    return this.core.drive.getUserDriveStats(id, ts() - day * 7, ts() + day)
                })
                .then((week) => {
                    stats.week = week;
                    reply(stats)
