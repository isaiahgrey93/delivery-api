...
module.exports = function(request, reply, next) {
	...
	let isScopePresent = request.payload.scope ? request.payload.scope.length > 0 : false;
	let isScopeAdmin = isScopePresent ? request.payload.scope.indexOf('admin') > -1 : false;
	...
	// only allow an non-admin scope to be set.
		...
		next(Boom.unauthorized(`Role <${auth.scope}> with user id: <${auth.id}> is not allowed to change another user\'s role to <admin>.`))
	...
	// only allow an admin to update non-admin's scope
		...
		.then((user) => {
			...
			if (user.scope.indexOf('admin') > -1) {
				next(Boom.unauthorized(`Role <${auth !== null ? auth.scope : 'guest'}> with user id: <${auth !== null ? auth.id : 'guest'}> is not allowed to change role <${user !== null ? user.scope : 'guest'}> with user id: <${user !== null ? user.id : 'guest'}> to new role.`))
			} else {
				next(null, true)
			}
