    ...
    init: (model) => {
      ...
      model.define('comparePassword', function (password) {
        return Bcrypt.compare(password, this.password)
          ...
          .catch(Bcrypt.MISMATCH_ERROR, () => Boom.badRequest("Values for [email] and [password] combination is incorrect."))
          .catch((err) => err);
      });
      ...
      model.pre('save', function (next) {
        ...
        return Core.user.findByEmail(this.email)
          .then((emailinUse) => {
            if(emailinUse) throw Boom.unauthorized("Value for [email] is already in use.");
