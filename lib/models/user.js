            ...
            model.pre('save', function (next) {
             ...
             model.changes().then((feed) => {
                feed.each((err, user) => {
                    ...
                    if (err) return err;
                    else if (user.isSaved() === false) return // deleted
                    else if (user.getOldValue() == null) return // created
                    else {  // updated
                        let prev = user.getOldValue();

                        if (!prev.connect_id && user.connect_id && user.connect_id.length > 0 && user.phone) {
                        ...
                        if (prev.status === 'unverified' && user.status === 'verified' && user.phone) {
                            Core.support.sms.send(
                                user.phone,
