...
module.exports = function () {
    ...
    const drive_statuses = ['unpaid', 'available', 'accepted', 'loading', 'driving', 'delivered', 'refunded'];
    ...
    return {
        ...
        init: (model) => {
            ...
            model.ensureIndex('origin', ReQL.row('route')('origin')('geo'), { geo: true })
            model.ensureIndex('destination', ReQL.row('route')('destination')('geo'), { geo: true })
            ...
            model.changes().then((feed) => {
                feed.each((err, drive) => {
                    ...
                    if (err) return err;
                        ...
                        if (prev.start_time === undefined && drive.status === 'loading') {
                            return Core.model('Drive').update(
                                Object.assign(drive, {
                                    start_time: new Date()
                                })
                            )
                        ...
                        if (prev.end_time === undefined && drive.status === 'delivered') {
                            return Core.model('Drive').update(
                                Object.assign(drive, {
                                    end_time: new Date()
                                })
