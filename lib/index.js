...
module.exports.register = (server, options, next) => {
  ...
  server.ext('onPreResponse', function (request, reply) {
    ...
    if (res instanceof Error && process.env.NODE_ENV === 'dev') {
      if (process.env.NODE_ENV === 'dev') {
        console.error('\n', `An error occured: ${(new Date()).toLocaleString('en-US')}`, '\n', request.response, '\n');
      }
      ...
      return reply({
        ...
        message : res.message
      }).code(res.output.payload.statusCode)
    }
  ...
  server.route([
    ...
    //
    {
      path: '/api/accounts/{account_type}',
      method: 'GET',
      config: Routes.stripe.getAccountList
    },
    {
      path: '/api/accounts/{account_type}/{user_id}',
      method: 'GET',
      config: Routes.stripe.getAccount
    },
    {
      path: '/api/accounts/{account_type}/{user_id}',
      method: 'POST',
      config: Routes.stripe.createAccount
    },
    {
      path: '/api/accounts/{account_type}/{user_id}',
      method: 'PUT',
      config: Routes.stripe.updateAccount
    },
    {
      path: '/api/accounts/{account_type}/{user_id}',
      method: 'DELETE',
      config: Routes.stripe.removeAccount
    },
    // 
    ...
    // 
    {
      ...
      method: 'POST',
      config: Routes.stripe.createCharge
    }
  ]);
