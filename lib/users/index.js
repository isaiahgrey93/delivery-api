...
class User {
    ...
    async generateMagicLink(data) {
        ...
        if (email) {
            ...
            let response = await resolve(
                this.emailLib.sendMagicLink(url, user)
            );

            if (response.error) {
                return {
                    error: new Error("Unable to send magic link.")
                };
            }
        ...
        if (phone) {
            ...
            let response = await resolve(
                this.telephonyLib.sendSms(
                    phone,
                    `Here is your magic link: ${url}?code=${user.magicLinkCode}`
                )
            );
    ...
    async authenticate(credentials, options = {}) {
        ...
        user = await resolve(this.libs.users.populate(user, options.populate));
    ...
    async filterBy(query, options = {}, geometry = {}) {
        let fetchedUsers = await resolve(this.store.filterBy(query, geometry));
