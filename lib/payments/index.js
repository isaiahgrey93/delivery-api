class Payment {
    ...
    async createCustomer(userId) {
        ...
        user = await resolve(
            this.libs.users.update({ payerAccountId: customer.id })
        );
    ...
    async addCardSource(customerId, cardDetails) {
        let cardSource = await resolve(
            this.gateway.addCardSource(customer, cardDetails)
        );
    ...
    async addBankSource(customerId, bankDetails) {
        let bankSource = await resolve(
            this.gateway.addBankSource(customer, bankDetails)
        );
    ...
    async authorizeCharge(source, amount, desc) {
        let charge = await resolve(
            this.gateway.authorizeCharge(source, amount, desc)
        );
