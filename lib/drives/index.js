...
class Drive {
    ...
    async create(drive) {
        let newDrive = new this.Entity(
            Object.assign({}, drive, { status: "unpaid" })
        );
    ...
    async complete(driveId) {
        ...
        let extensions = drive.support ? Object.values(drive.support) : [];
        ...
        let transfer = await resolve(
            this.libs.payments.createTransfer(
                authorizedCost,
                drive.payment.chargeId,
                drive.driver.driver.payeeAccountId,
                {
                    drive: drive.id,
    ...
    async estimate(driveId, complete = false) {
        ...
        let { base: pricePerHour = 0, mile: pricePerMile = 0 } = truckPrice;
        ...
        if (driveType === "consumer") {
            ...
            let mileage = (distance * pricePerMile).toFixed(2);
